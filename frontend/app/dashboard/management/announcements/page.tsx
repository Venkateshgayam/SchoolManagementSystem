...
import { Megaphone, Pin, Calendar, Save, X } from "lucide-react";
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {announcements.slice().sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime()).map((a) => (
                  <tr key={a.id}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-right"><button onClick={() => togglePin(a)} className="text-xs text-blue-600 hover:text-blue-800">{a.is_pinned ? "Unpin" : "Pin"}</button></td>
                  </tr>
                ))}
              </tbody>
            </table>
