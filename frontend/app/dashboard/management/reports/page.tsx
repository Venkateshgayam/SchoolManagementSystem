...
export default function ManagementReportsPage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const [students, teachers, classes, subjects, attendance, grades, fees, leaves, announcements, notifications] = await Promise.all([
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="card"><Receipt className="h-5 w-5 text-emerald-600 mb-1" /><p className="text-2xl font-bold">${revenue.toFixed(2)}</p><p className="text-sm text-gray-600">Collected Fees</p></div>
        ...
        <div className="card"><FileText className="h-5 w-5 text-pink-600 mb-1" /><p className="text-2xl font-bold">{pinnedAnnouncements}</p><p className="text-sm text-gray-600">Pinned Announcements</p></div>
