...
export default function GradesPage() {
  ...
  const canCreate = can("grade:create");
  const canEdit = can("grade:update");
  ...
  return (
    <div>
      <PageHeader
        ...
        icon={BarChart3}
        action={canCreate ? <button onClick={openCreate} className="btn-primary flex items-center gap-1"><Plus className="h-4 w-4" /> Add Grade</button> : undefined}
      />
      <div className="card mb-6">
        <StatCard title="Average" value={`${avg.toFixed(2)}%`} icon={BarChart3} trend={`${grades.length} grade(s)`} />
      </div>
      {grades.length === 0 ? (
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50"><tr>
                ...
                {canEdit && <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase" />}
              </tr></thead>
